...
contract KiloTestSuite {
    ...
    function testBuffToken() external payable {
        ...
        (charge, activeCharge, value, , ) = digil.tokenCharge(fireTokenId);
        Assert.equal(charge, 0, "Invalid final buff Destination charge");
        Assert.equal(activeCharge, 0, "Invalid final buff Destination active charge");

        digil.chargeToken{value: incrementalValue * 100}(activeTokenId, coinMultiplier * 100);

        (charge, activeCharge, value, , ) = digil.tokenCharge(activeTokenId);
        Assert.equal(charge, 0, "Invalid final charge Source charge");
        Assert.equal(activeCharge, coinMultiplier * 1205, "Invalid final charge Source active charge");

        (charge, activeCharge, value, , ) = digil.tokenCharge(fireTokenId);
        Assert.equal(charge, coinMultiplier * 10, "Invalid final charge Destination charge");
        Assert.equal(activeCharge, 0, "Invalid final charge Destination active charge");
    }
}